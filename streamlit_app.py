...
st.set_page_config(page_title="Home Depot Search Relevance",
                   layout="wide")  # Set the page title and icon

st.title("Home Depot Search Relevance")  
st.text(
    """Please enter your search query in the text box below to fetch the top 10 relevant products and its relevance score""") 
import pandas as pd
import numpy as np
import regex as re
# !pip install nltk
...
#mainly refered from --> https://github.com/kriz17/Home-Depot-Product-Search-Relevance/blob/master/App_Files/app.py
...
def P_word(word, N=sum(WORDS.values())):
    
    return WORDS[word] / N


def prob_correction(word):
    return max(spell_corr_candidates(word), key=P_word)


def spell_corr_candidates(word):
    # "Generates possible spelling corrections for a given word"
    return (known_words([word]) or known_words(edits1(word)) or known_words(edits2(word)) or set([word]))
...
def edits1(word):
    ...
    replaces = [L + c + R[1:] for L, R in splits if R for c in letter]
    inserts = [L + c + R for L, R in splits for c in letter]
...
def corrected_term(term):
    ...
    temp = [prob_correction(word) for word in temp]
    return ' '.join(temp)
...
def preprocessing(sent):
    sent = sent.replace('in.', ' inch ')  # 'in.' will be turned to 'in' in this is not done
    words = re.split(r'\W+', sent)
    words = [word.lower() for word in words]
    res = re.sub("[A-Za-z]+", lambda ele: " " + ele[0] + " ",
                 ' '.join(words))  # adding space between numericals and alphabets in a string
    cleaned = standardize_units(res)
    cleaned = ' '.join(cleaned.split())  # extra whitespaces removal
    return cleaned
...
def count_num_in_search_attribute(sent1, sent2):
    search_term_tokens = sent1.split(' ')
    numbers_in_search_term = set([i for i in search_term_tokens if i.isdigit()])
    attr_tokens = sent2.split(' ')
    numbers_attr = set([i for i in attr_tokens if i.isdigit()])
    return len(numbers_in_search_term & numbers_attr)


def common_word_count_ngram(sent1, sent2, ngram=1, edit_distance_thresshold=0):
    sent1 = sent1.split(' ')
    sent2 = sent2.split(' ')
    ngrams_1 = set(ngrams(sent1, ngram))
    ngrams_2 = set(ngrams(sent2, ngram))
...
def cosine_similarity_sentence(sent1, sent2):
    A = set(sent1.split())
    B = set(sent2.split())
    numerator = len(A & B)
...
def has_brand(search_term, brand, edit_dis_thresshold=1):
...
def min_jaccard_coefficient(sent1, sent2):
...
def minimum_edit_distance(sent1, sent2):
    sent1 = sent1.split(' ')
    sent2 = sent2.split(' ')
    minimum_distance = 99999
    for i in sent1:
...
def get_mean_jcc_searchterm_and_title(sent1, sent2):
    search_term_word = sent1.strip().split(' ')
    desc_word = sent2.strip().split(' ')
    mean = 0.0
...
def get_mean_jcc_searchterm_and_desc(sent1, sent2):
    search_term_word = sent1.strip().split(' ')
    desc_word = sent2.strip().split(' ')
    mean = 0.0
    for i in search_term_word:
...
def get_mean_jcc_searchterm_and_attr(sent1, sent2):
    search_term_word = sent1.strip().split(' ')
    desc_word = sent2.strip().split(' ')
    mean = 0.0
    for i in search_term_word:
...
def get_search_relevance(test_set):
    ...
    data1['numbers_in_attr'] = data1.progress_apply(
        lambda x: count_num_in_search_attribute(x['corrected_search'], x['attributes']), axis=1)
    data1['numbers_in_title'] = data1.progress_apply(
        lambda x: count_num_in_search_attribute(x['corrected_search'], x['title']), axis=1)
    data1['numbers_in_desc'] = data1.progress_apply(
        lambda x: count_num_in_search_attribute(x['corrected_search'], x['description']), axis=1)

    data1['title_common_word_count'] = data1.progress_apply(
        lambda x: common_word_count_ngram(x['corrected_search'], x['title'], edit_distance_thresshold=1), axis=1)
    data1['desc_common_word_count'] = data1.progress_apply(
        lambda x: common_word_count_ngram(x['corrected_search'], x['description'], edit_distance_thresshold=1), axis=1)
    data1['attr_common_words_count'] = data1.progress_apply(
        lambda x: common_word_count_ngram(x['corrected_search'], x['attributes'], edit_distance_thresshold=1), axis=1)

    data1['title_common_word_count_bigram'] = data1.progress_apply(
        lambda x: common_word_count_ngram(x['corrected_search'], x['title'], ngram=2, edit_distance_thresshold=4),
        axis=1)
    data1['desc_common_word_count_bigram'] = data1.progress_apply(
        lambda x: common_word_count_ngram(x['corrected_search'], x['description'], ngram=2,
                                           edit_distance_thresshold=4), axis=1)
    data1['attr_common_word_count_bigram'] = data1.progress_apply(
        lambda x: common_word_count_ngram(x['corrected_search'], x['attributes'], ngram=2, edit_distance_thresshold=4),
        axis=1)
    ...
    data2['cosine_search_term_Title'] = data2.progress_apply(
        lambda row: cosine_similarity_sentence(row['corrected_search'], row['title']), axis=1)
    data2['cosine_search_term_desc'] = data2.progress_apply(
        lambda row: cosine_similarity_sentence(row['corrected_search'], row['description']), axis=1)

    data2['search_term_contains_brand'] = data2.progress_apply(
        lambda x: has_brand(x['corrected_search'], x['brand']), axis=1)

    data2['min_jcc_brand_with_search_term'] = data2.progress_apply(
        lambda x: min_jaccard_coefficient(x['corrected_search'], x['brand']), axis=1)
    ...
    data2['min_jcc_title_with_search_term'] = data2.progress_apply(
        lambda x: min_jaccard_coefficient(x['corrected_search'], x['title']), axis=1)
...
with st.form("Search Prediction "):  
    query = st.text_input("Enter search query here")  
    if st.form_submit_button("Submit"):  
        results = (main(query, 10))  
